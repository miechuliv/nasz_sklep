...
class Cart {
		...
		if (!$this->data) {
			foreach ($this->session->data['cart'] as $key => $quantity) {
				...
				if ($product_query->num_rows) {
						...
						if ($option_query->num_rows) {
							if ($option_query->row['type'] == 'select' || $option_query->row['type'] == 'radio' || $option_query->row['type'] == 'image') {
								...
								if ($option_value_query->num_rows) {
                                    ...
                                    // jeśli druk tak i liczba podanych kolorów
                                    if($option_query->row['option_id'] == 3 AND $option_value_query->row['option_value_id'] == 1)
                                    {
                                        $l_kolorow = 0;
                                        // nadruk tak, sprawdzamy liczbę kolorów
                                        foreach ($options as $key_3 => $value) {
                                            $opt_q = $this->db->query("SELECT po.product_option_id, po.option_id, od.name, o.type FROM " . DB_PREFIX . "product_option po LEFT JOIN `" . DB_PREFIX . "option` o ON (po.option_id = o.option_id) LEFT JOIN " . DB_PREFIX . "option_description od ON (o.option_id = od.option_id) WHERE po.product_option_id = '" . (int)$key_3 . "' AND po.product_id = '" . (int)$product_id . "' AND od.language_id = '" . (int)$this->config->get('config_language_id') . "'");
                                            $opt_val_q = $this->db->query("SELECT pov.option_value_id, ovd.name, pov.quantity, pov.subtract, pov.price, pov.price_prefix, pov.points, pov.points_prefix, pov.weight, pov.weight_prefix FROM " . DB_PREFIX . "product_option_value pov LEFT JOIN " . DB_PREFIX . "option_value ov ON (pov.option_value_id = ov.option_value_id) LEFT JOIN " . DB_PREFIX . "option_value_description ovd ON (ov.option_value_id = ovd.option_value_id) WHERE pov.product_option_value_id = '" . (int)$value . "' AND pov.product_option_id = '" . (int)$key_3 . "' AND ovd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
                                        ...
                                        $option_price = $this->getProductKosztDruku($product_id,$quantity,$l_kolorow);
                    ...
                    $p = $this->getAviableDiscountByProductId($product_id,$quantity);


                    if($p)
                    {
                        $price = (float)($price*(1-(float)($p/100)));
                    }
    ...
    public function getProductKosztDruku($product_id , $quantity, $colors)
    {
        ...
        if($grupaDruku->num_rows)
        {
            ...
            $q = $this->db->query("SELECT * FROM `".DB_PREFIX."grupy_druku`
             ...
             `grupa_druku` = '".$this->db->escape($grupaDruku->row['text'])."' AND
              `colors` = '".(int)$colors."' ORDER BY `from` DESC ");
    ...
    public function getAviableDiscountByProductId($product_id,$quantity)
    {
