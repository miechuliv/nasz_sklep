...
class ControllerCheckoutCart extends Controller {
	...
	public function index() {
    	...
    	if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
            ...
            $this->data['error_warning_text'] = $this->language->get('error_stock');
    ...
    public function changeQuantity()
    {
        ...
        if($product_info AND isset($this->request->post['change']))
        {
            ...
            {
                $quantity  = $this->cart->getQuantity($key) + 1;
            ...
            if($this->request->post['change']=='replace' AND isset($this->request->post['quantity']))
            {
            ...
            $quantity  = $this->cart->getQuantity($key_m);

            if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {

                  //$json['product_total'] = $this->currency->format($this->tax->calculate($product_info['price'], $product_info['tax_class_id'], $this->config->get('config_tax')) * $quantity);

                $products = $this->cart->getProducts();
                foreach($products as $product)
                {
                    if($product['key'] == $key_m)
                    {
                        $json['product_total'] = $this->currency->format($product['total']);
                        $json['price'] = $this->currency->format($product['price']);
                        $json['stock'] = $product['stock'];
                        $json['key'] = $product['key'];
                    }
                }
            } else {
	...
	public function add() {
		...
		if ($product_info) {			
			...
			foreach ($product_options as $product_option) {
                ...
                $opt = $this->model_catalog_product->getOptionByProductOptionValue($this->request->post['product_id'],$option[$product_option['product_option_id']],$this->config->get('config_language_id'));


                // ustawiono druk na tak
                if($opt['option_id'] == 3 AND $opt['option_value_id'] == 1)
                {
                    $druk  = true;
                }
            ...
            if($druk AND !$kolory)
            {
                ...
                $opt = $this->model_catalog_product->getProductOptionByOption($this->request->post['product_id'],2,$this->config->get('config_language_id'));


                if($opt)
                {
                    $json['error']['option'][$opt['product_option_id']] = sprintf($this->language->get('error_required'), $opt['name']);
                }
			...
			if (!$json) {
                ...
                $json['title'] = $this->language->get('text_add_to_cart');
                $json['thumb'] = $this->model_tool_image->resize($product_info['image'], 120, 120);
                ...
                $json['see_cart']=$this->language->get('text_see_cart');
                ...
                // przekierowanie na podstrone jeśli fruk
                if($druk)
                {
                    $json['redirect'] = str_replace('&amp;', '&', $this->url->link('product/druk' , '&product_id='.$product_id));
                }
                else
                {
